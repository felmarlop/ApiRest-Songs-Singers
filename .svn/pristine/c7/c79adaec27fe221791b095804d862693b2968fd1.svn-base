...
public class SongServlet extends HttpServlet {
	...
	private void getSongs(HttpServletRequest req, HttpServletResponse res)
			throws IOException {
	...
	@SuppressWarnings("static-access")
	private void postSong(HttpServletRequest req, HttpServletResponse res)
			throws IOException {
		...
		try {
			...
			if (mapaSingers.containsKey(s.getAutor())) {
				if (!mapaSongs.containsKey(s.getName())) {
					...
					mapaSongs.put(s.getName(), s);
					datastore.put(e);
					System.out.println("Song añadida");

				} else {
					res.setStatus(res.SC_FORBIDDEN);
					System.out.println("ERROR al añadir Song: Ya existe ");
				}
			} else {
				res.setStatus(res.SC_FORBIDDEN);
				System.out.println("ERROR al añadir Song: No existe el autor ");
			}

		} catch (Exception ex) {
			res.setStatus(res.SC_INTERNAL_SERVER_ERROR);
			System.out.println("ERROR al añadir Song: " + ex.getMessage());
		}

	}

	@SuppressWarnings("static-access")
	private void deleteSongs(HttpServletRequest req, HttpServletResponse res)
			throws IOException {
		try {
			if (!mapaSongs.isEmpty()) {
				...
				System.out
						.println("ERROR al borrar canciones: No existe ninguna ");
			}
			mapaSongs.clear();
	...
	@SuppressWarnings("static-access")
	private void procesarRecurso(String method, String resource,
		...
		if (!mapaSongs.containsKey(resource)) {
			res.setStatus(res.SC_NOT_FOUND);
			System.out
					.println("ERROR: Ningun nombre se corresponde con el Path");
			return;
		} else {
			switch (method) {
			case "GET":
				getSong(resource, req, res);
				break;
			case "PUT":
				updateSong(resource, req, res);
				break;
			case "DELETE":
				deleteSong(resource);
				break;
	...
	@SuppressWarnings("static-access")
	private void getSong(String resource, HttpServletRequest req,
			HttpServletResponse res) throws IOException {
		Entity salida = null;
		Song s = null;
		try {
	...
	@SuppressWarnings("static-access")
	public void updateSong(String resource, HttpServletRequest req,
			HttpServletResponse res) throws IOException {
		...
		if (actualizada == null) {
			...
			res.setStatus(res.SC_FORBIDDEN);
			System.out
					.println("ERROR: El Path no se corresponde con el nombre de Song");
			return;
		} else if (!mapaSingers.containsKey(actualizada.getAutor())) {
			res.setStatus(res.SC_NOT_FOUND);
			System.out.println("ERROR al actualizar: El autor no existe");
			return;
		} else {
			try {
				...
				System.out.println("Song actualizada: "
						+ actualizada.toString());
	...
	private Song entityToSong(Entity e) {
		Song s = new Song((String) e.getProperty("name"),
	...
	private Entity buscarSong(String resource) {
		...
		Query q = new Query("Song").setFilter(new Query.FilterPredicate("name",
